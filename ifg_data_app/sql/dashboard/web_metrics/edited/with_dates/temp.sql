-- Unit of analysis: Unique publication (downloadable file or page with content type of 'Publication') x date combinations
select distinct
    da.url_most_common,
    da.file_path_latest,
    da.file_name_clean,
...
select
    ...
    a.author Author,
    p.topic Topic,
    m.page_views [Page views (pages downloadable from)],
    m.event_count Downloads
from corporate.content_basic_metadata_canonical bm      -- Start with a page and build from there
    ...
    outer apply (       -- Attach metrics for all pages from which the file has been downloaded. 1:n following application of aggregation (1:n x m before this a file can be downloadable from multiple URLs)
        select
            pv.date,
...
where
    ...
    m.date between '1900-01-01' and '9999-12-31';
